# Hearth — Property Rental & Maintenance

Hearth is a compact property-operations application that replaces rent spreadsheets and maintenance sticky notes. It implements the ten goals in this assignment with server-enforced roles, a relational data model, an append-only audit timeline, server-side maintenance search, bulk rent processing, CSV export, dashboards, and monthly renewable alerts.
...
Open `http://localhost:3000`. The database and realistic demo data are created automatically.

| Role | Email | Password |
|---|---|---|
| Property manager | `manager@hearth.test` | `DemoPass123!` |
| Contractor | `alex@hearth.test` | `DemoPass123!` |
| Contractor | `sam@hearth.test` | `DemoPass123!` |

Useful commands:

```bash
pnpm check   # type-check and run domain/integration tests
pnpm build   # compile production JavaScript
pnpm start   # run the compiled service
```

Configuration is documented in `.env.example`. SQLite data is stored in `data/hearth.db` by default and is deliberately ignored by Git.
